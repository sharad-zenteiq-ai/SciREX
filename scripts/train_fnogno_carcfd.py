...
def main():
    ...
    if config.wandb_log:
        wandb_name = f"car-pressure_{config.query_res[0]}"
        wandb.init(
            project="Auto-FNOGNO",
            name=wandb_name,
    ...
    dataset = CarCFDDataset(
        ...
        in_gno_radius=config.gno_radius, # Note: dataset asks for in/out, passing gno_radius to both safely
        out_gno_radius=config.gno_radius,
        neighbor_cache_dir=config.neighbor_cache_dir,
        use_cache=True
    )
    ...
    model = FNOGNO(
        ...
        gno_transform_type=config.gno_transform_type,
    ...
    ckpt_path = os.path.join(ckpt_dir, "fnogno_carcfd_params.pkl")
    ...
    for epoch in range(config.epochs):
        ...
        for batch in dataset.get_batch("train", batch_size=config.batch_size):
        ...
        test_loss = 0.0
        test_steps = 0
        for batch in dataset.get_batch("test", batch_size=config.batch_size):
            batch_geom, batch_queries, batch_out_queries, batch_lat_features, batch_y, batch_in_nb, batch_out_nb = preprocess_cfd_sample(batch, config)
